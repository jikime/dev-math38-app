...
import { 
  ...
  SavePaperRequest,
  SavePaperResponse,
  AddonPaperGenerateParams,
  ErrorReportListResponse,
  SchoolPaperSearchParams,
  SchoolPaperSearchResult
} from '@/net/core/types';
import { M38Lecture, LectureVO, GenerateAcademyPaperVO, GeneratedPaper } from '@/types/typings';
...
export class MainApiService extends BaseApiService<'main'> {
   ...
   * @returns 내 강좌 목록
   */
  async getMyLectures(): Promise<LectureVO[]> {
  ...
  //  * @returns 학생 목록
  //  */
  // async getLectureStudents(lectureId: string | null): Promise<LectureStudent[]> {
  ...
  //  * @returns 시험지 목록
  //  */
  // async getLecturePapersByType(lectureId: string, type: string): Promise<LecturePaper[]> {
   ...
   * @returns 오류 신고 목록
   */
  async getUserErrorReports(page: number = 0, size: number = 10): Promise<ErrorReportListResponse> {
   ...
   * @returns 추가 결과
   */
  async addPapersFromSaveLecture(lectureId: string, saveLectureId: string, paperIds: string[]): Promise<SimpleResponse> {
   ...
   * @returns 추가 결과
   */
  async addProvidedPapers(lectureId: string, paperIds: string[]): Promise<SimpleResponse> {
   ...
   * @returns 저장 결과
   */
  async saveStudyPaper(paper: SavePaperRequest): Promise<boolean> {
   ...
   * @returns 저장 결과
   */
  async saveManualPaper(type: string, paper: SavePaperRequest): Promise<SavePaperResponse> {
   ...
   * @returns 추가 결과
   */
  async addManualPaperInLecture(lectureId: string, type: string, paper: GeneratedPaper): Promise<SavePaperResponse> {
   ...
   * @returns 강좌 정보
   */
  async getLectureByType(type: string, id: string): Promise<M38Lecture | LectureVO> {
   ...
   * @returns 마지막 인덱스 정보
   */
  async getLastIndexInfo(type: string, lectureId: string): Promise<{ lastIndex: number; lastTitle?: string }> {
   ...
   * @returns 추가된 시험지 정보
   */
  async addAcademyPaperInLecture(lectureId: string, academyPaperInput: GenerateAcademyPaperVO): Promise<SavePaperResponse> {
   ...
   * @returns 시험지 정보
   */
  async getManualPaper(paperId: string): Promise<SavePaperRequest> {
   ...
   * @returns 생성 결과
   */
  async generateWorkbookPaper(data: SavePaperRequest): Promise<SimpleResponse> {
   ...
   * @returns 생성 결과
   */
  async generateWorkbookPaperInRangeSkills(data: SavePaperRequest): Promise<SimpleResponse> {
   ...
   * @returns 지역 목록
   */
  async getRegions(): Promise<Array<{ id: number; name: string; districts?: Array<{ id: number; name: string }> }>> {
   ...
   * @returns 학교 목록
   */
  async getExamSchools(districtId: number, schoolType: string): Promise<Array<{ schoolId: string; schoolName: string; address?: string }>> {
   ...
   * @returns 시험지 검색 결과
   */
  async searchExamPapers(filter: SchoolPaperSearchParams, pageRequest: { page: number; size: number }): Promise<SchoolPaperSearchResult> {
   ...
   * @returns 생성 결과
   */
  async createAddonPaper(param: AddonPaperGenerateParams): Promise<SavePaperResponse> {
