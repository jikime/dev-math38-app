/**
 * API Hook Factory
 * API Registry를 기반으로 타입 안전한 React Query Hook을 생성합니다.
 */

import { useApiQuery, useApiQueryPost, UseApiQueryOptions } from '@/net/core/api/useApiQuery';
import { UseQueryResult } from '@tanstack/react-query';
import { ApiServer } from '@/net/core/registry/ApiRegistry';

// Hook 생성기 타입
export type HookCreator<TArgs extends any[], TData> = (
  ...args: [...TArgs, UseApiQueryOptions?]
...
export function createGetHook<TArgs extends any[], TData>(
  ...
  return (...args) => {
    ...
    const options = hasOptions ? args[args.length - 1] as UseApiQueryOptions : undefined;
    ...
    return useApiQuery<TData>(server, url, options);
  };
}
...
export function createPostHook<TArgs extends any[], TBody, TData>(
  ...
  return (body, ...args) => {
    ...
    return useApiQueryPost<TData>(server, url, body, options);
  };
}
