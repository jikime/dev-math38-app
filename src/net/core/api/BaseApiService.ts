...
import { ApiRequestOptions, IApiService, PaginatedResponse, SimpleResponse } from '@/net/core/types';
import { API_REGISTRY, ApiRegistry, ApiServer } from '@/net/core/registry/ApiRegistry';
...
export abstract class BaseApiService<TServer extends ApiServer = ApiServer> implements IApiService {
  ...
  constructor(apiClient: ApiClient, server: TServer | string) {
    this.apiClient = apiClient;
    // string 타입도 허용하여 동적 서버 ID 지원
    this.urls = API_REGISTRY[server as TServer];
  }
