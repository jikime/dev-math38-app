...
export function PrescriptionSheet({ 
  ...
  // paperIds 변경 감지 및 로깅
  useEffect(() => {
    console.log('📋 paperIds 변경 감지:', {
      paperIds,
      paperIdsCount: paperIds?.length || 0,
      selectedStudentsCount: selectedStudents.length,
      lectureId,
      willCallAPI: !!(lectureId && selectedStudents.length > 0)
    })
    
    // 선택된 학생이 있고 paperIds가 변경되었다면 API 호출이 이루어져야 함
    if (lectureId && selectedStudents.length > 0 && paperIds) {
      console.log('🔄 paperIds 변경으로 인한 API 호출 예정')
      // 약간의 지연을 두고 API 재호출 (React Query가 파라미터 변경을 감지할 시간 확보)
      setTimeout(() => {
        if (paperSolveCountsParams) {
          refetchSolveCounts()
          console.log('🚀 API 강제 재호출 실행')
        }
      }, 100)
    }
  }, [paperIds, selectedStudents.length, lectureId, paperSolveCountsParams, refetchSolveCounts])
  ...
  // multiplies 변경 시 학생 선택 유지 (초기화하지 않음)
  // filteredStudents가 변경될 때 selectAllStudents 상태 업데이트
  useEffect(() => {
    if (filteredStudents.length > 0) {
      const allSelected = filteredStudents.every(student => selectedStudents.includes(student.userId))
          ...
          <div className="flex-shrink-0 px-6 py-4 bg-gray-25 dark:bg-gray-950/30 border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-4">
              <div className="flex items-center gap-2 min-w-0 flex-1 min-w-[200px]">
                <span className="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap flex-shrink-0">시험지명</span>
                <Input 
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <span className="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap flex-shrink-0">출제순서</span>
                        ...
                        {selectedStudents.includes(student.userId) && (
                          solveCountsLoading ? (
                            ...
                            <div className="flex flex-col gap-1 items-center">
                              <div className="bg-white dark:bg-gray-800 border-2 border-red-500 rounded-lg px-2 py-1.5">
                                <div className="text-xs font-bold text-red-600 dark:text-red-400">{student.wrongAnswers || 0}</div>
                                <div className="text-[10px] text-red-600 dark:text-red-400">{ratio(student.wrongAnswers || 0, student.totalQuestions)}</div>
                              </div>
                            </div>
                          ) : null
                        )}
                      ...
                      <TableCell className="text-center text-xs hidden sm:table-cell">
                        {selectedStudents.includes(student.userId) && (
                          solveCountsLoading ? (
                            ...
                            <div className="flex flex-col gap-1 items-center">
                              <div className="bg-white dark:bg-gray-800 border-2 border-orange-500 rounded-lg px-2 py-1.5">
                                <div className="text-xs font-bold text-orange-600 dark:text-orange-400">{student.partialWrong || 0}</div>
                                <div className="text-[10px] text-orange-600 dark:text-orange-400">{ratio(student.partialWrong || 0, student.totalQuestions)}</div>
                              </div>
                            </div>
                          ) : null
                        )}
                      ...
                      <TableCell className="text-center text-xs hidden sm:table-cell">
                        {selectedStudents.includes(student.userId) && (
                          solveCountsLoading ? (
                            ...
                            <div className="flex flex-col gap-1 items-center">
                              <div className="bg-white dark:bg-gray-800 border-2 border-yellow-500 rounded-lg px-2 py-1.5">
                                <div className="text-xs font-bold text-yellow-600 dark:text-yellow-500">{student.partialCorrect || 0}</div>
                                <div className="text-[10px] text-yellow-600 dark:text-yellow-500">{ratio(student.partialCorrect || 0, student.totalQuestions)}</div>
                              </div>
                            </div>
                          ) : null
                        )}
                      ...
                      <TableCell className="text-center text-xs">
                        {selectedStudents.includes(student.userId) && (
                          solveCountsLoading ? (
                            ...
                            <div className="flex flex-col gap-1 items-center">
                              <div className="bg-white dark:bg-gray-800 border-2 border-green-500 rounded-lg px-2 py-1.5">
                                <div className="text-xs font-bold text-green-600 dark:text-green-400">{student.correct || 0}</div>
                                <div className="text-[10px] text-green-500 dark:text-green-400">{ratio(student.correct || 0, student.totalQuestions)}</div>
                              </div>
                            </div>
                          ) : null
                        )}
                      ...
                      <div className="p-3">
                        {solveCountsLoading ? (
                          ...
                          <div className="flex flex-col gap-1 text-center">
                            <div className="bg-white dark:bg-gray-800 border-2 border-red-500 rounded-lg px-2 py-1.5 min-w-[50px]">
                              <div className="text-sm font-bold text-red-600 dark:text-red-400">{student.wrongAnswers || 0}</div>
                          ...
                          {/* 부분오답 */}
                          <div className="flex flex-col gap-1 text-center">
                            <div className="bg-white dark:bg-gray-800 border-2 border-orange-500 rounded-lg px-2 py-1.5 min-w-[50px]">
                              <div className="text-sm font-bold text-orange-600 dark:text-orange-400">{student.partialWrong || 0}</div>
                          ...
                          {/* 부분정답 */}
                          <div className="flex flex-col gap-1 text-center">
                            <div className="bg-white dark:bg-gray-800 border-2 border-yellow-500 rounded-lg px-2 py-1.5 min-w-[50px]">
                              <div className="text-sm font-bold text-yellow-600 dark:text-yellow-500">{student.partialCorrect || 0}</div>
                              <div className="text-[10px] text-yellow-600 dark:text-yellow-500 font-medium">
                          ...
                          {/* 정답 */}
                          <div className="flex flex-col gap-1 text-center">
                            <div className="bg-white dark:bg-gray-800 border-2 border-green-500 rounded-lg px-2 py-1.5 min-w-[50px]">
                              <div className="text-sm font-bold text-green-600 dark:text-green-400">{student.correct || 0}</div>
                              <div className="text-[10px] text-green-500 dark:text-green-400 font-medium">
